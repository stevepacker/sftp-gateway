...
var log          = require('./logger')('sftpd.log');
...
server.onConnect = function(client, info) {
    server.log.info('Client connected!', info);
    ...
    client.on('ready', function() {
        server.log.info('Client authenticated');
        ...
        switch (ctx.method) {
            case 'password':
                if (options.password == ctx.password) {
                    server.log.info('Client has authenticated via password');
            ...
            case 'publickey':
                ...
                if (pubKey.fulltype != ctx.key.algo) {
                    server.log.info('pubKey algo did not match', pubKey.fulltype, ctx.key.algo);
                    break;
                }

                if (0 !== pubKey.public.compare(ctx.key.data)) {
                    server.log.info('pubKey data did not match', pubKey.public, ctx.key.data);
                    break;
                }

                if (! ctx.signature) {
                    // if no signature present, the client is just checking the public key
                    server.log.info('Client is checking the public key');
                    return ctx.accept();
                }
                ...
                if (verify.verify(pubKey.publicOrig, ctx.signature)) {
                    server.log.info('Client has authenticated via publickey');
                    client.username = ctx.username;
                    return ctx.accept();
                }
                break;
...
server.onSftp = function(accept, reject, client) {
    server.log.info('SFTP session started', client.username);
    var sftpStream = accept();

    var cmds = [
        'OPEN', 'READ', 'FSTAT', 'FSETSTAT', 'CLOSE', 'OPENDIR', 'READDIR', 'LSTAT', 'STAT',
    ...
    _(cmds).forEach(function(eventName) {
        sftpStream.on(eventName, function(reqid) {
            server.log.debug(eventName, arguments);
        });
    });
    ...
    sftpStream.on('CLOSE', function(reqid, handle) {
        ...
        if (server.openFiles[filename]) {
            server.emit('fileUploadDone', filename, client);
        }
    ...
    sftpStream.on('WRITE', function(reqid, filenameBuffer, offset, data) {
        ...
        if (size > server.maxUploadBytes) {
            server.log.warn('File too large: ', size);
            return sftpStream.status(reqid, STATUS_CODE.FAILURE);
        }

        if (! server.openFiles[filename]) {
            server.log.warn('File is not opened: ' + size);
            return sftpStream.status(reqid, STATUS_CODE.FAILURE);
        }

        // broadcast to anyone listening
        server.emit('fileUploadPart', filename, data, offset, client);
